...
function check2(e) {
  ...
  let isValidPassword = passVal.length >= 8;
  let isValidName = nameVal.length >= 3;
  let isValidPhone = phonePattern.test(phoneVal);
  let isValidUniversity = univVal.length > 0;
  let isValidID = idPattern.test(idVal);

  // Only prevent default if any field is invalid
  if (!isValidEmail || !isValidPassword || !isValidName || !isValidPhone || !isValidUniversity || !isValidID) {
    e.preventDefault();
  }
  ...
  if (!isValidEmail) {
    ...
    email.style.cssText =
      "animation: move .4s;border-color: var(--main-color);";
    lable_email.innerText = "Invalid Email Format";
    firEmail = false;
  }
  ...
  if (!isValidPassword) {
    ...
    lable_pass.innerText = "Password must be at least 8 characters";
    firPass = false;
  }
  ...
  if (!isValidName) {
    ...
    label_name.innerText = "Name must be at least 3 characters";
    firName = false;
  }
  ...
  if (!isValidPhone) {
    ...
    phone.style.cssText =
      "animation: move .4s;border-color: var(--main-color);";
    label_phone.innerText = "Invalid Phone Number (010, 011, 012, 015)";
    firPhone = false;
  }
  ...
  if (!isValidUniversity) {
    ...
    label_univ.innerText = "University name is required";
    firUniv = false;
  }
  ...
  if (!isValidID) {
    ...
    label_id.innerText = "Invalid ID Number (14 digits)";
    firID = false;
  }
}
